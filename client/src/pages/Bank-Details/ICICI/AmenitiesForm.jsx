...
// // ─── MAIN COMPONENT ───────────────────────────────────────────────────────────
// const AmenitiesForm = ({ data, editData, extractedData, onSave, onSaveAndNext, saving, stateRef,isPlot,isResidential, hideProjectAmenities }) => {
...
// AmenitiesForm.jsx — Final: Unit Amenities (dropdowns) + Project Amenities (checkboxes)
import React, { useState, useEffect, useRef } from "react";
...
const AmenitiesForm = ({
  ...
  const [form, setForm] = useState({
    // ── Project Amenities (for Non-Residential & Residential both) ──
    furnished: false,
    greenBuildingCertified: false,
    hvac: false,
    lift: false,
    ...
    // ── Unit Amenities (for Residential non-Plot only) ──
    unitFlooring: "",
    unitPlumbing: "",
    unitCornerPlot: "",
    unitPaintQuality: "",
    unitViewsFacing: "",
    unitAnyOtherQualityFacing: "",
    unitDoors: "",
    unitElectricalFittings: "",
    unitWindows: "",
    unitKitchen: "",

    // ── Unit Amenities → Project Amenities sub-section (CHECKBOXES) ──
    unitAmphitheatre: false,
    unitFireExit: false,
    unitLift: false,
    unitCctvCameras: false,
    unitGarden: false,
    unitPipeGasConnection: false,
    unitClubHouse: false,
    unitGymnasium: false,
    unitPlayArea: false,
    unitCommunityHall: false,
    unitJoggingTrack: false,
    unitSwimmingPool: false,
    unitAnyOtherProjectAmenities: "",
    unitLoadingPlatformAvailable: false,
    unitSeatingCapacity: "",
    unitCeilingHeight: "",
  ...
  useEffect(() => {
    const src = data && Object.keys(data).length > 0 ? data : editData || {};
    const autofillData = extractedData || {};
    if (Object.keys(src).length === 0 && Object.keys(autofillData).length === 0)
      return;
    const { images: imgs, ...rest } = src;
    setForm((p) => ({ ...p, ...rest, ...autofillData }));
    if (imgs) setImages(imgs);
  ...
  // ─── DROPDOWN OPTIONS ────────────────────────────────────────────────────────
  const flooringOptions = ["Granite", "Marble", "Mozaic", "Tiles", "Vitrified"];
  const plumbingOptions = ["Concealed", "Open"];
  const cornerPlotOptions = ["Yes", "No"];
  const paintQualityOptions = ["Good", "Moderate", "Poor"];
  const viewsFacingOptions = [
    "Club House Facing",
    "Garden Facing",
    "Internal Building Facing",
    "Lake Facing",
    "River Facing",
    "Road Facing",
    "Sea View Facing",
  ...
  const electricalFittingsOptions = ["Concealed", "Opened"];
  const windowsOptions = ["Openable", "Sliding"];
  const kitchenOptions = ["Modular", "Normal"];
  ...
  return (
    <>
      ...
      <div className="am-wrap">
        {/* ════════════════════════════════════════════════════════════════════
            AMENITIES CARD – conditionally shown
        ════════════════════════════════════════════════════════════════════ */}
        {amenitiesType !== "none" && (
          <div className="am-card">
            {amenitiesType === "project" ? (
              // ─── PROJECT AMENITIES (Non‑Residential only) ───
              <>
                <div className="am-section-title">Project Amenities</div>
                <div className="am-chk-grid-4">
                  <label className="am-checkbox">
                  ...
                  <label className="am-checkbox">
                    <input
                      type="checkbox"
                      checked={form.greenBuildingCertified}
                      onChange={() => hChk("greenBuildingCertified")}
                    />{" "}
                    Green Building Certified
                  </label>
                  <label className="am-checkbox">
                  ...
                  <label className="am-checkbox">
                    <input
                      type="checkbox"
                      checked={form.meetingRoom}
                      onChange={() => hChk("meetingRoom")}
                    />{" "}
                    Meeting Room
                  </label>
                  <label className="am-checkbox">
                  ...
                  <label className="am-checkbox">
                    <input
                      type="checkbox"
                      checked={form.viewVista}
                      onChange={() => hChk("viewVista")}
                    />{" "}
                    View Vista
                  </label>
                  <label className="am-checkbox">
                    <input
                      type="checkbox"
                      checked={form.washroom}
                      onChange={() => hChk("washroom")}
                    />{" "}
                    Washroom
                  </label>
                </div>
                ...
                <div className="am-chk-grid-2" style={{ marginTop: 20 }}>
                  <label className="am-checkbox">
                    <input
                      type="checkbox"
                      checked={form.frontageAvailable}
                      onChange={() => hChk("frontageAvailable")}
                    />{" "}
                    Frontage Available
                  </label>
                  <label className="am-checkbox">
                    <input
                      type="checkbox"
                      checked={form.loftAvailable}
                      onChange={() => hChk("loftAvailable")}
                    />{" "}
                    Loft Available
                  </label>
                </div>
                  ...
                  <div className="am-radio-row" style={{ marginTop: 6 }}>
                    <label className="am-radio">
                      <input
                        ...
                        name="footFall"
                        value="high"
                        checked={form.footFall === "high"}
                        onChange={() => hc("footFall", "high")}
                      />{" "}
                      High
                    </label>
                    <label className="am-radio">
                      <input
                        ...
                        name="footFall"
                        value="low"
                        checked={form.footFall === "low"}
                        onChange={() => hc("footFall", "low")}
                      />{" "}
                      Low
                    </label>
                    <label className="am-radio">
                      <input
                        ...
                        name="footFall"
                        value="medium"
                        checked={form.footFall === "medium"}
                        onChange={() => hc("footFall", "medium")}
                      />{" "}
                      Medium
                    </label>
                  </div>
                ...
                <div style={{ marginTop: 20 }}>
                  <label className="am-checkbox">
                    <input
                      type="checkbox"
                      checked={form.loadingPlatformAvailable}
                      onChange={() => hChk("loadingPlatformAvailable")}
                    />{" "}
                    Loading Platform Available
                  </label>
                </div>
              ...
              // // ─── UNIT AMENITIES (Residential + non‑Plot) ───
              // <>
              //   <div className="am-section-title">Unit Amenities</div>

              //   {/* Row 1: Flooring, Plumbing, Corner Plot */}
              //   <div className="am-chk-grid-4" style={{ alignItems: 'end' }}>
              //     <div>
              //       <span className="am-field-label">Flooring</span>
              //       <select className="am-input" value={form.unitFlooring || ""} onChange={(e) => hc("unitFlooring", e.target.value)}>
              //         <option value="">Select</option>
              //         {flooringOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
              //       </select>
              //     </div>
              //     <div>
              //       <span className="am-field-label">Plumbing</span>
              //       <select className="am-input" value={form.unitPlumbing || ""} onChange={(e) => hc("unitPlumbing", e.target.value)}>
              //         <option value="">Select</option>
              //         {plumbingOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
              //       </select>
              //     </div>
              //     <div>
              //       <span className="am-field-label">Corner Plot</span>
              //       <select className="am-input" value={form.unitCornerPlot || ""} onChange={(e) => hc("unitCornerPlot", e.target.value)}>
              //         <option value="">Select</option>
              //         {cornerPlotOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
              //       </select>
              //     </div>
              //   </div>

              //   {/* Row 2: Paint Quality, Views & Facing, Any Other? */}
              //   <div style={{ marginTop: 16 }}>
              //     <div className="am-chk-grid-4" style={{ alignItems: 'end' }}>
              //       <div>
              //         <span className="am-field-label">Paint Quality</span>
              //         <select className="am-input" value={form.unitPaintQuality || ""} onChange={(e) => hc("unitPaintQuality", e.target.value)}>
              //           <option value="">Select</option>
              //           {paintQualityOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
              //         </select>
              //       </div>
              //       <div>
              //         <span className="am-field-label">Views &amp; Facing</span>
              //         <select className="am-input" value={form.unitViewsFacing || ""} onChange={(e) => hc("unitViewsFacing", e.target.value)}>
              //           <option value="">Select</option>
              //           {viewsFacingOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
              //         </select>
              //       </div>
              //     </div>
              //     <div style={{ marginTop: 8 }}>
              //       <span className="am-field-label">Any Other?</span>
              //       <input className="am-input" value={form.unitAnyOtherQualityFacing || ""} onChange={(e) => hc("unitAnyOtherQualityFacing", e.target.value)} placeholder="Enter other options" />
              //     </div>
              //   </div>

              //   {/* Row 3: Doors, Electrical Fittings, Windows, Kitchen */}
              //   <div style={{ marginTop: 16 }}>
              //     <div className="am-chk-grid-4" style={{ alignItems: 'end' }}>
              //       <div>
              //         <span className="am-field-label">Doors</span>
              //         <select className="am-input" value={form.unitDoors || ""} onChange={(e) => hc("unitDoors", e.target.value)}>
              //           <option value="">Select</option>
              //           {doorsOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
              //         </select>
              //       </div>
              //       <div>
              //         <span className="am-field-label">Electrical Fittings</span>
              //         <select className="am-input" value={form.unitElectricalFittings || ""} onChange={(e) => hc("unitElectricalFittings", e.target.value)}>
              //           <option value="">Select</option>
              //           {electricalFittingsOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
              //         </select>
              //       </div>
              //       <div>
              //         <span className="am-field-label">Windows</span>
              //         <select className="am-input" value={form.unitWindows || ""} onChange={(e) => hc("unitWindows", e.target.value)}>
              //           <option value="">Select</option>
              //           {windowsOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
              //         </select>
              //       </div>
              //       <div>
              //         <span className="am-field-label">Kitchen</span>
              //         <select className="am-input" value={form.unitKitchen || ""} onChange={(e) => hc("unitKitchen", e.target.value)}>
              //           <option value="">Select</option>
              //           {kitchenOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
              //         </select>
              //       </div>
              //     </div>
              //   </div>
              //   {/* ******************************************************8 */}
              //   {/* ─── PROJECT AMENITIES (sub‑section) – CHECKBOXES ─── */}
              //   <div style={{ marginTop: 28, borderTop: "1px solid #f0f0f0", paddingTop: 20 }}>
              //     <div className="am-sub-label" style={{ fontWeight: 600, fontSize: 14, color: "#e8472b" }}>Project Amenities</div>
              //     <div className="am-chk-grid-4" style={{ marginTop: 8 }}>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.unitAmphitheatre} onChange={() => hChk("unitAmphitheatre")} /> Amphitheatre</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.unitFireExit} onChange={() => hChk("unitFireExit")} /> Fire Exit</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.unitLift} onChange={() => hChk("unitLift")} /> Lift</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.unitCctvCameras} onChange={() => hChk("unitCctvCameras")} /> CCTV Cameras</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.unitGarden} onChange={() => hChk("unitGarden")} /> Garden</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.unitPipeGasConnection} onChange={() => hChk("unitPipeGasConnection")} /> Pipe Gas Connection</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.unitClubHouse} onChange={() => hChk("unitClubHouse")} /> Club House</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.unitGymnasium} onChange={() => hChk("unitGymnasium")} /> Gymnasium</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.unitPlayArea} onChange={() => hChk("unitPlayArea")} /> Play Area</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.unitCommunityHall} onChange={() => hChk("unitCommunityHall")} /> Community Hall</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.unitJoggingTrack} onChange={() => hChk("unitJoggingTrack")} /> Jogging Track</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.unitSwimmingPool} onChange={() => hChk("unitSwimmingPool")} /> Swimming Pool</label>
              //     </div>

              //     <div style={{ marginTop: 8 }}>
              //       <span className="am-field-label">Any Other?</span>
              //       <input className="am-input" value={form.unitAnyOtherProjectAmenities || ""} onChange={(e) => hc("unitAnyOtherProjectAmenities", e.target.value)} placeholder="Enter other amenities" />
              //     </div>
              //   </div>

              //   {/* Loading Platform Available – CHECKBOX */}
              //   <div style={{ marginTop: 16 }}>
              //     <label className="am-checkbox"><input type="checkbox" checked={form.unitLoadingPlatformAvailable} onChange={() => hChk("unitLoadingPlatformAvailable")} /> Loading Platform Available</label>
              //   </div>

              //   {/* Seating Capacity */}
              //   <div style={{ marginTop: 16 }}>
              //     <span className="am-field-label">Seating Capacity</span>
              //     <input className="am-input am-narrow" value={form.unitSeatingCapacity} onChange={(e) => hc("unitSeatingCapacity", e.target.value)} placeholder="Enter capacity" />
              //   </div>

              //   {/* Ceiling Height */}
              //   <div style={{ marginTop: 16 }}>
              //     <span className="am-field-label">Ceiling Height (Ft)</span>
              //     <input className="am-input am-narrow" value={form.unitCeilingHeight} onChange={(e) => hc("unitCeilingHeight", e.target.value)} placeholder="Enter height" />
              //   </div>

              //   {/* ─── PROJECT AMENITIES (ORIGINAL CHECKBOXES – for Residential) ─── */}
              //   <div style={{ marginTop: 28, borderTop: "1px solid #f0f0f0", paddingTop: 20 }}>
              //     <div className="am-section-title">Project Amenities</div>
              //     <div className="am-chk-grid-4">
              //       <label className="am-checkbox"><input type="checkbox" checked={form.furnished} onChange={() => hChk("furnished")} /> Furnished</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.greenBuildingCertified} onChange={() => hChk("greenBuildingCertified")} /> Green Building Certified</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.hvac} onChange={() => hChk("hvac")} /> Hvac</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.lift} onChange={() => hChk("lift")} /> Lift</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.meetingRoom} onChange={() => hChk("meetingRoom")} /> Meeting Room</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.pantry} onChange={() => hChk("pantry")} /> Pantry</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.parking} onChange={() => hChk("parking")} /> Parking</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.viewVista} onChange={() => hChk("viewVista")} /> View Vista</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.washroom} onChange={() => hChk("washroom")} /> Washroom</label>
              //     </div>

              //     <div style={{ marginTop: 20 }}>
              //       <span className="am-field-label">Seating Capacity</span>
              //       <input className="am-input am-narrow" value={form.seatingCapacity} onChange={(e) => hc("seatingCapacity", e.target.value)} />
              //     </div>

              //     <div className="am-chk-grid-2" style={{ marginTop: 20 }}>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.frontageAvailable} onChange={() => hChk("frontageAvailable")} /> Frontage Available</label>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.loftAvailable} onChange={() => hChk("loftAvailable")} /> Loft Available</label>
              //     </div>

              //     <div style={{ marginTop: 20 }}>
              //       <span className="am-field-label">Foot Fall</span>
              //       <div className="am-radio-row" style={{ marginTop: 6 }}>
              //         <label className="am-radio"><input type="radio" name="footFall" value="high" checked={form.footFall === "high"} onChange={() => hc("footFall", "high")} /> High</label>
              //         <label className="am-radio"><input type="radio" name="footFall" value="low" checked={form.footFall === "low"} onChange={() => hc("footFall", "low")} /> Low</label>
              //         <label className="am-radio"><input type="radio" name="footFall" value="medium" checked={form.footFall === "medium"} onChange={() => hc("footFall", "medium")} /> Medium</label>
              //       </div>
              //     </div>

              //     <div style={{ marginTop: 20 }}>
              //       <span className="am-field-label">Ceiling Height (Ft)</span>
              //       <input className="am-input am-narrow" value={form.ceilingHeight} onChange={(e) => hc("ceilingHeight", e.target.value)} />
              //     </div>

              //     <div style={{ marginTop: 20 }}>
              //       <label className="am-checkbox"><input type="checkbox" checked={form.loadingPlatformAvailable} onChange={() => hChk("loadingPlatformAvailable")} /> Loading Platform Available</label>
              //     </div>
              //   </div>
              // </>
              ...
              <>
                ...
                <div className="am-chk-grid-4" style={{ alignItems: "end" }}>
                  <div>
                    ...
                    <select
                      className="am-input"
                      value={form.unitFlooring || ""}
                      onChange={(e) => hc("unitFlooring", e.target.value)}
                    >
                      <option value="">Select</option>
                  ...
                  <div>
                    ...
                    <select
                      className="am-input"
                      value={form.unitPlumbing || ""}
                      onChange={(e) => hc("unitPlumbing", e.target.value)}
                    >
                      <option value="">Select</option>
                      {plumbingOptions.map((opt) => (
                        <option key={opt} value={opt}>
                          {opt}
                        </option>
                      ))}
                  ...
                  <div>
                    ...
                    <select
                      className="am-input"
                      value={form.unitCornerPlot || ""}
                      onChange={(e) => hc("unitCornerPlot", e.target.value)}
                    >
                      <option value="">Select</option>
                      {cornerPlotOptions.map((opt) => (
                        <option key={opt} value={opt}>
                          {opt}
                        </option>
                      ))}
                ...
                <div style={{ marginTop: 16 }}>
                  <div className="am-chk-grid-4" style={{ alignItems: "end" }}>
                    <div>
                      ...
                      <select
                        className="am-input"
                        value={form.unitPaintQuality || ""}
                        onChange={(e) => hc("unitPaintQuality", e.target.value)}
                      >
                        <option value="">Select</option>
                        {paintQualityOptions.map((opt) => (
                          <option key={opt} value={opt}>
                            {opt}
                          </option>
                        ))}
                    ...
                    <div>
                      ...
                      <select
                        className="am-input"
                        value={form.unitViewsFacing || ""}
                        onChange={(e) => hc("unitViewsFacing", e.target.value)}
                      >
                        <option value="">Select</option>
                        {viewsFacingOptions.map((opt) => (
                          <option key={opt} value={opt}>
                            {opt}
                          </option>
                        ))}
                    ...
                    <div>
                      ...
                      <select
                        className="am-input"
                        value={form.unitDoors || ""}
                        onChange={(e) => hc("unitDoors", e.target.value)}
                      >
                        <option value="">Select</option>
                ...
                <div style={{ marginTop: 16 }}>
                  <div className="am-chk-grid-4" style={{ alignItems: "end" }}>
                    ...
                    <div>
                      ...
                      <select
                        className="am-input"
                        value={form.unitElectricalFittings || ""}
                        onChange={(e) =>
                          hc("unitElectricalFittings", e.target.value)
                        }
                      >
                        <option value="">Select</option>
                        {electricalFittingsOptions.map((opt) => (
                          <option key={opt} value={opt}>
                            {opt}
                          </option>
                        ))}
                    ...
                    <div>
                      ...
                      <select
                        className="am-input"
                        value={form.unitWindows || ""}
                        onChange={(e) => hc("unitWindows", e.target.value)}
                      >
                        <option value="">Select</option>
                    ...
                    <div>
                      ...
                      <select
                        className="am-input"
                        value={form.unitKitchen || ""}
                        onChange={(e) => hc("unitKitchen", e.target.value)}
                      >
                        <option value="">Select</option>
                    ...
                    <input
                      className="am-input"
                      value={form.unitAnyOtherQualityFacing || ""}
                      onChange={(e) =>
                        hc("unitAnyOtherQualityFacing", e.target.value)
                      }
                ...
                >
                  <div
                    className="am-sub-label"
                    style={{ fontWeight: 600, fontSize: 14, color: "#e8472b" }}
                  >
                    Project Amenities
                  </div>
                  <div className="am-chk-grid-4" style={{ marginTop: 8 }}>
                    <label className="am-checkbox">
                      <input
                        type="checkbox"
                        checked={form.unitAmphitheatre}
                        onChange={() => hChk("unitAmphitheatre")}
                      />{" "}
                      Amphitheatre
                    </label>
                    <label className="am-checkbox">
                      <input
                        type="checkbox"
                        checked={form.unitFireExit}
                        onChange={() => hChk("unitFireExit")}
                      />{" "}
                      Fire Exit
                    </label>
                    <label className="am-checkbox">
                      <input
                        type="checkbox"
                        checked={form.unitLift}
                        onChange={() => hChk("unitLift")}
                      />{" "}
                      Lift
                    </label>
                    <label className="am-checkbox">
                      <input
                        type="checkbox"
                        checked={form.unitCctvCameras}
                        onChange={() => hChk("unitCctvCameras")}
                      />{" "}
                      CCTV Cameras
                    </label>
                    <label className="am-checkbox">
                      <input
                        type="checkbox"
                        checked={form.unitGarden}
                        onChange={() => hChk("unitGarden")}
                      />{" "}
                      Garden
                    </label>
                    <label className="am-checkbox">
                      <input
                        type="checkbox"
                        checked={form.unitPipeGasConnection}
                        onChange={() => hChk("unitPipeGasConnection")}
                      />{" "}
                      Pipe Gas Connection
                    </label>
                    <label className="am-checkbox">
                      <input
                        type="checkbox"
                        checked={form.unitClubHouse}
                        onChange={() => hChk("unitClubHouse")}
                      />{" "}
                      Club House
                    </label>
                    <label className="am-checkbox">
                      <input
                        type="checkbox"
                        checked={form.unitGymnasium}
                        onChange={() => hChk("unitGymnasium")}
                      />{" "}
                      Gymnasium
                    </label>
                    <label className="am-checkbox">
                      <input
                        type="checkbox"
                        checked={form.unitPlayArea}
                        onChange={() => hChk("unitPlayArea")}
                      />{" "}
                      Play Area
                    </label>
                    <label className="am-checkbox">
                      <input
                        type="checkbox"
                        checked={form.unitCommunityHall}
                        onChange={() => hChk("unitCommunityHall")}
                      />{" "}
                      Community Hall
                    </label>
                    <label className="am-checkbox">
                      <input
                        type="checkbox"
                        checked={form.unitJoggingTrack}
                        onChange={() => hChk("unitJoggingTrack")}
                      />{" "}
                      Jogging Track
                    </label>
                    <label className="am-checkbox">
                      <input
                        type="checkbox"
                        checked={form.unitSwimmingPool}
                        onChange={() => hChk("unitSwimmingPool")}
                      />{" "}
                      Swimming Pool
                    </label>
                  </div>
                  <div style={{ marginTop: 8 }}>
                    ...
                    <input
                      className="am-input"
                      value={form.unitAnyOtherProjectAmenities || ""}
                      onChange={(e) =>
                        hc("unitAnyOtherProjectAmenities", e.target.value)
                      }
                ...
                <div style={{ marginTop: 16 }}>
                  <label className="am-checkbox">
                    <input
                      type="checkbox"
                      checked={form.unitLoadingPlatformAvailable}
                      onChange={() => hChk("unitLoadingPlatformAvailable")}
                    />{" "}
                    Loading Platform Available
                  </label>
                </div>
                ...
                <div style={{ marginTop: 16 }}>
                  ...
                  <input
                    className="am-input am-narrow"
                    value={form.unitSeatingCapacity}
                    onChange={(e) => hc("unitSeatingCapacity", e.target.value)}
                ...
                <div style={{ marginTop: 16 }}>
                  ...
                  <input
                    className="am-input am-narrow"
                    value={form.unitCeilingHeight}
                    onChange={(e) => hc("unitCeilingHeight", e.target.value)}
        ...
        {/* ══ SURROUNDING AMENITIES (always visible) ══ */}
        <div className="am-card">
          <div className="am-section-title">Surrounding Amenities</div>

          <div className="am-sub-label" style={{ marginTop: 0 }}>
          ...
          <div className="am-radio-row">
            <label className="am-radio">
              <input
                ...
                value="developed"
                checked={form.infraSurroundings === "developed"}
                onChange={() => hc("infraSurroundings", "developed")}
              />{" "}
              Developed
            </label>
            <label className="am-radio">
              <input
                ...
                name="infraSurroundings"
                value="developing"
                checked={form.infraSurroundings === "developing"}
                onChange={() => hc("infraSurroundings", "developing")}
              />{" "}
              Developing
            </label>
            <label className="am-radio">
              <input
                ...
                checked={form.infraSurroundings === "underDeveloped"}
                onChange={() => hc("infraSurroundings", "underDeveloped")}
              />{" "}
              Under Developed
            </label>
          </div>
          ...
          <div className="am-radio-row">
            <label className="am-radio">
              <input
                ...
                name="accessibility"
                value="average"
                checked={form.accessibility === "average"}
                onChange={() => hc("accessibility", "average")}
              />{" "}
              Average
            </label>
            <label className="am-radio">
              <input
                ...
                name="accessibility"
                value="good"
                checked={form.accessibility === "good"}
                onChange={() => hc("accessibility", "good")}
              />{" "}
              Good
            </label>
            <label className="am-radio">
              <input
                ...
                name="accessibility"
                value="poor"
                checked={form.accessibility === "poor"}
                onChange={() => hc("accessibility", "poor")}
              />{" "}
              Poor
            </label>
            <label className="am-radio">
              <input
                ...
                name="accessibility"
                value="veryGood"
                checked={form.accessibility === "veryGood"}
                onChange={() => hc("accessibility", "veryGood")}
              />{" "}
              Very Good
            </label>
            <label className="am-radio">
              <input
                ...
                name="accessibility"
                value="veryPoor"
                checked={form.accessibility === "veryPoor"}
                onChange={() => hc("accessibility", "veryPoor")}
              />{" "}
              Very Poor
            </label>
          </div>
        ...
        <div className="am-card">
          ...
          <div className="am-chk-grid-2">
            <label className="am-checkbox">
              <input
                type="checkbox"
                checked={form.financialDistrict}
                onChange={() => hChk("financialDistrict")}
              />{" "}
              Financial District
            </label>
            <label className="am-checkbox">
          ...
          <div className="am-radio-row">
            <label className="am-radio">
              <input
                type="radio"
                name="surroundingOccupancy"
                value="fullyOccupied"
                checked={form.surroundingOccupancy === "fullyOccupied"}
                onChange={() => hc("surroundingOccupancy", "fullyOccupied")}
              />{" "}
              Fully Occupied
            </label>
            <label className="am-radio">
              <input
                ...
                name="surroundingOccupancy"
                value="largelyVacant"
                checked={form.surroundingOccupancy === "largelyVacant"}
                onChange={() => hc("surroundingOccupancy", "largelyVacant")}
              />{" "}
              Largely Vacant
            </label>
            <label className="am-radio">
              <input
                ...
                name="surroundingOccupancy"
                value="partOccupied"
                checked={form.surroundingOccupancy === "partOccupied"}
                onChange={() => hc("surroundingOccupancy", "partOccupied")}
              />{" "}
              Part Occupied
            </label>
          </div>
          ...
          <div className="am-three-col" style={{ marginTop: 20 }}>
            ...
            <div>
              <span className="am-field-label">
                Type Of Industry Occupying Surrounding Premise
              </span>
              <input
                className="am-input"
                value={form.typeOfIndustry}
        ...
        {/* ══ AMENITIES IMAGES (always visible) ══ */}
        <div className="am-card">
          <div className="am-section-title">Amenities Images</div>
          ...
          />
          <p className="am-upload-hint">
            * Supported file formats are JPEG, JPG, PNG (Maximum file size 500
            KB)
          </p>
              ...
              <div className="am-modal-right">
                ...
                <div className="am-modal-field">
                  ...
                  <input
                    value={modalForm.longitude}
                    onChange={(e) =>
                      setModalForm((p) => ({ ...p, longitude: e.target.value }))
                    }
                  />
                </div>
                <div className="am-modal-field">
                  ...
                  <input
                    value={modalForm.latitude}
                    onChange={(e) =>
                      setModalForm((p) => ({ ...p, latitude: e.target.value }))
                    }
                  />
                </div>
              </div>
              {images.length > 1 && (
                <div className="am-modal-nav" onClick={() => navModal(1)}>
                  ›
                </div>
              )}
            </div>
            <div className="am-modal-footer">
              <button className="am-btn" onClick={closeModal}>
                Cancel
              </button>
              <button className="am-btn primary" onClick={updateModal}>
                Update
              </button>
            </div>
          </div>
